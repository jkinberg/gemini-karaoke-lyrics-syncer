...
- **High-Accuracy Synchronization**: Utilizes a two-step process with **Gemini 2.5 Pro** to analyze an audio file and align original lyrics with millisecond precision, ensuring top-tier accuracy.
...
3.  **Generate Step 1: High-Fidelity Timing**: When "Generate Synced Files" is clicked, the app first sends the audio file and the **original lyrics** to the powerful **Gemini 2.5 Pro** model. Its sole focus is to perform a highly accurate, word-level audio-to-text alignment, creating a "source of truth" timed data file.
